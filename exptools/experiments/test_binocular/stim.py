...
class RandomBarFrameStimulus(object):
    ...
    def __init__(self, screen, trial, session, config):
        ...
        width_hbar_squares = int(self.session.deg2pix(config['width_barsquare']) / square_size_pix)
        ...
        bar_matrix2 = (np.random.random((height_vbar_squares, width_vbar_squares)) > 0.5) * 2 - 1
        bar_matrix3 = (np.random.random((height_hbar_squares, width_hbar_squares)) > 0.5) * 2 - 1
        bar_matrix4 = (np.random.random((height_vbar_squares, width_vbar_squares)) > 0.5) * 2 - 1
        ...
        bar1 = visual.ImageStim(self.screen,
                                bar_matrix1, 
                                size=(width_hbar_pix, height_hbar_pix), 
                                pos=(0, self.session.deg2pix(config['height_barsquare'])/2.))

        bar2 = visual.ImageStim(self.screen, 
                                bar_matrix2, 
                                size=(width_vbar_pix, height_vbar_pix), 
                                pos=(self.session.deg2pix(config['width_barsquare'])/2., 0))

        bar3 = visual.ImageStim(self.screen, 
                                bar_matrix3, 
                                size=(width_hbar_pix, height_hbar_pix), 
                                pos=(0, -self.session.deg2pix(config['height_barsquare'])/2.))
